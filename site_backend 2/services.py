...
def get_sum_very_first_by_date(start_date=date.today() - timedelta(days=31), end_date=date.today()):
    """
    ...
    obj = models.VeryFirst.objects.filter(date_end__gte=start_date, date_end__lte=end_date).aggregate(Sum('price'))
    print(obj)
    try:
        return obj['price__sum']
    except:
...
def get_history():
    return models.UsersHistory.objects.all().order_by("date").reverse().values()
